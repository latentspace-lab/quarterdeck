...
// single-player path and deliberately allowed, so a forgotten parameter
// doesn't break the game. Server and tests always pass a seed.
...
export const ROLES: RoleDef[] = [
   ...
   { id: "powder", name: "Powder Boys", short: "Powder Boys", share: 0.05 },
];

/** Where a role is found - that decides who gets hit. */
export type Where = "hull" | "deck" | "rig" | "below";

// hull  = at the hull (splinters from the struck section)
// deck  = on open deck (grape, musketry)
// rig   = aloft in the rigging (chain shot)
// below = below deck (relatively safe)
const EXPOSURE: Record<string, Record<Where, number>> = {
   officer: { hull: 0.6, deck: 1.4, rig: 0.2, below: 0.3 },
   gun: { hull: 1.8, deck: 0.9, rig: 0.1, below: 0.5 },
   top: { hull: 0.4, deck: 1.1, rig: 2.6, below: 0.2 },
   marine: { hull: 0.5, deck: 1.6, rig: 0.5, below: 0.2 },
   carpenter: { hull: 0.7, deck: 0.4, rig: 0.1, below: 1.4 },
   powder: { hull: 0.9, deck: 0.5, rig: 0.1, below: 1.0 },
};
...
export class Crew {
   ...
   /** Set the random source later (e.g. when taking over a room). */
   setRng(rng: Rng): void {
      this.rng = rng;
   }
   ...
   // ------------------------------------------------------------------
   // Losses
   // n      = number of people hit
   // where  = "hull" | "deck" | "rig" | "below"
   ...
   hit(n: number, where: Where = "hull"): CasualtyResult {
      ...
      // Weights from location and remaining crew
      const ids: string[] = [];
      const w: number[] = [];
      let sum = 0;
   ...
   // Below about 60 % crew, guns are consolidated: fewer guns,
   // but the remaining ones stay reasonably fast.
   ...
   /** Sail handling: topmen set and reef sails */
   sailHandling(): number {
      return clamp(0.25 + 0.75 * this.fraction("top"), 0.2, 1);
   }
   /** Pumping and plugging leaks */
   pumping(): number {
      return clamp(0.2 + 0.8 * this.fraction("carpenter"), 0.15, 1);
   }
   /** Rudder and chain of command */
   command(): number {
      return clamp(0.4 + 0.6 * this.fraction("officer"), 0.35, 1);
   }
   /** Combat morale: collapses when losses and loss of leadership combine */
   morale(): number {
